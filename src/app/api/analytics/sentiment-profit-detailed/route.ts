...
export async function GET(request: NextRequest) {
  try {
    ...
    // Query to get sentiment scores grouped by supermarket branch with profit data
    ...
    const groupedData = rawData.reduce((acc: any, item: any) => {
      ...
      const profit = parseFloat(item.supermarket_branches.profit);
    ...
    const allProfits = chartData.map(d => d.profit);

    const averageSentimentScore = allSentimentScores.reduce((sum, score) => sum + score, 0) / allSentimentScores.length;
    const averageProfit = allProfits.reduce((sum, profit) => sum + profit, 0) / allProfits.length;

    // Calculate correlation coefficient
    const correlationCoefficient = calculateCorrelation(allProfits, allSentimentScores);
